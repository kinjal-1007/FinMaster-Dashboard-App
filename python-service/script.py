...
def upload_file():
    ...
    # Calculate LTV using full data
    ltv_data = df_full.groupby("Customer ID").agg(
    ...
    # Compute lifespan in years (minimum 1 year to avoid divide by zero)
    ltv_data["lifespan_years"] = (
        ((ltv_data["last_purchase"] - ltv_data["first_purchase"]).dt.days / 365)
        .round()
        .clip(lower=1)
    )

    # Average Order Value (AOV)
    ltv_data["aov"] = ltv_data["total_revenue"] / ltv_data["total_orders"]

    ltv_data["estimated_ltv"] = ltv_data["aov"] * ltv_data["total_orders"] / ltv_data["lifespan_years"] * ltv_data["lifespan_years"]
    # Simplifies to total_revenue, but this structure allows flexibility

    # Format for merging
    ltv_data = ltv_data[["Customer ID", "estimated_ltv"]]
    ltv_data["estimated_ltv"] = ltv_data["estimated_ltv"].apply(format_currency)

    # Merge with current customer_data
    customer_data = customer_data.merge(ltv_data, on="Customer ID", how="left")
    ...
    customer_data["estimated_ltv"] = customer_data["estimated_ltv"].fillna(0).apply(safe_format_currency)
...
def index():
    return jsonify({"message": "Welcome to the Sales Data Analysis API!"})
